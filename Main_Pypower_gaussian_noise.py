...
for index in range(len(r2)):
    ...
    target = torch.load('./data/pypower14/test_target_r2_'+ str(r2[index]) + '.pt').to(dev)
    observation = torch.load('./data/pypower14/test_observation_r2_' + str(r2[index]) + '.pt').to(dev)
    print(target.size())
    print(observation.size())
    target = target[:N_T,:,:T_test].float()
    observation = observation[:N_T,:,:T_test].float()
    nl_train_input = torch.load('./data/pypower14/train_observation_r2_' + str(r2[index]) + '.pt').to(dev)
    nl_cv_input = torch.load('./data/pypower14/valid_observation_r2_' + str(r2[index]) + '.pt').to(dev)
    nl_test_input = observation
    nl_train_target = torch.load('./data/pypower14/train_target_r2_' + str(r2[index]) + '.pt').to(dev)
    nl_cv_target = torch.load('./data/pypower14/valid_target_r2_' + str(r2[index]) + '.pt').to(dev)
